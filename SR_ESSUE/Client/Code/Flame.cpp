...
CFlame::CFlame( LPDIRECT3DDEVICE9 pDevice )
...
HRESULT CFlame::Initialize( const wstring& wstrKey, D3DXVECTOR3 vOrigin )
{
	...
	wsprintf(szBuf, L"../bin/Resources/Texture/Etc/Fire%d.png", iRanTex);
...
void CFlame::ResetParticle( Engine::ATTRIBUTE* pAttribute )
{
	pAttribute->bAlive = true;

	Engine::GetRandomVector(&pAttribute->vPos, &D3DXVECTOR3(-1.f, -1.f, -1.f), &D3DXVECTOR3(1.f, 1.f, 1.f));

	pAttribute->vPos += m_vOrigin;

	Engine::GetRandomVector(&pAttribute->vVelocity, &D3DXVECTOR3(0.f, 1.f, 0.f), &D3DXVECTOR3(0.f, 5.f, 0.f));
	pAttribute->vVelocity *= 2.f;

	// 번개 색상 - 빨강
	pAttribute->vColor = D3DXCOLOR(0.9f, 0.1f, 0.1f, 1.f);
...
CFlame* CFlame::Create( LPDIRECT3DDEVICE9 pDevice, const wstring& wstrKey, D3DXVECTOR3 vOrigin )
{
	CFlame* pFlame = new CFlame(pDevice);

	if (FAILED(pFlame->Initialize(wstrKey, vOrigin)))
		Engine::Safe_Delete(pFlame);
