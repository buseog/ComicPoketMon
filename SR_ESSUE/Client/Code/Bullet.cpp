...
HRESULT CBullet::Initialize( const wstring& wstrKey, D3DXVECTOR3 vOrigin, D3DXVECTOR3 vDir, int iCount )
{
...
void CBullet::ResetParticle( Engine::ATTRIBUTE* pAttribute )
{
	pAttribute->bAlive = true;

	pAttribute->vPos = m_vDir * 1.f;
	pAttribute->vPos += m_vOrigin;
	pAttribute->vPos.y += 3;

	pAttribute->vVelocity = m_vDir;
	pAttribute->vVelocity *= 40.f;

	pAttribute->vColor = D3DXCOLOR(0.f, 1.f, 0.9f, 1.f);
...
CBullet* CBullet::Create(LPDIRECT3DDEVICE9 pDevice, const wstring& wstrKey, D3DXVECTOR3 vOrigin, D3DXVECTOR3 vDir, int iCount)
{
	CBullet* pBullet = new CBullet(pDevice);

	if (FAILED(pBullet->Initialize(wstrKey, vOrigin, vDir, iCount)))
		Engine::Safe_Delete(pBullet);
